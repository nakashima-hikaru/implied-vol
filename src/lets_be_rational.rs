...
use crate::constants::{DENORMALISATION_CUTOFF, FOURTH_ROOT_DBL_EPSILON, LN_TWO_PI, PI_OVER_SIX, SIXTEENTH_ROOT_DBL_EPSILON, SQRT_DBL_MAX, SQRT_DBL_MIN, SQRT_ONE_OVER_THREE, SQRT_PI_OVER_TWO, SQRT_THREE, SQRT_THREE_OVER_THIRD_ROOT_TWO_PI, SQRT_TWO_PI, TWO_PI, TWO_PI_OVER_SQRT_TWENTY_SEVEN, VOLATILITY_VALUE_TO_SIGNAL_PRICE_IS_ABOVE_MAXIMUM, VOLATILITY_VALUE_TO_SIGNAL_PRICE_IS_BELOW_INTRINSIC};
...
fn normalised_intrinsic(x: f64, q: bool) -> f64 {
    ...
    if x2 < 98.0 * FOURTH_ROOT_DBL_EPSILON {
        return (if q { 1.0 } else { -1.0 }) * x * (1.0 + x2 * ((1.0 / 24.0) + x2 * ((1.0 / 1920.0) + x2 * ((1.0 / 322560.0) + (1.0 / 92897280.0) * x2)))).abs().max(0.0);
    }
    let b_max = (0.5 * x).exp();
    let one_over_b_max = b_max.recip();
    (if q { 1.0 } else { -1.0 }) * (b_max - one_over_b_max).abs().max(0.0)
}
...
fn ln_normalised_vega(x: f64, s: f64) -> f64 {
    ...
    if ax <= 0.0 {
        ...
        -(LN_TWO_PI / 2.0) - 0.5 * (square(x / s) + square(0.5 * s))
    }
}
...
fn compute_f_lower_map_and_first_two_derivatives(x: f64, s: f64) -> (f64, f64, f64) {
    ...
    let phi = norm_pdf(z);
    let fpp = PI_OVER_SIX * y / (s2 * s) * phi_m * (8.0 * SQRT_THREE * s * ax + (3.0 * s2 * (s2 - 8.0) - 8.0 * x * x) * phi_m / phi) * (2.0 * y + 0.25 * s2).exp();
...
fn unchecked_normalised_implied_volatility_from_a_transformed_rational_guess_with_limited_iterations(
    mut beta: f64, mut x: f64, q: bool, n: i32,
) -> f64 {
    if (q && (x > 0.0)) || (!q && (x < 0.0)) {
...
fn implied_volatility_from_a_transformed_rational_guess_with_limited_iterations(
    ...
    if (q && (x > 0.0)) || (!q && (x < 0.0)) {  // q && x > 0.0 or !q && x < 0.0
