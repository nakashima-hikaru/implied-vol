...
fn phi_tilde_times_x(x: f64) -> f64 {
    if x.abs() <= 0.612_003_180_962_480_7 {
        let h = (x.powi(2) - 1.872_739_467_540_974_8E-1) * 5.339_771_053_755_08;
    ...
    if x >= -3.5 {
        ...
        return (-0.5 * x.powi(2)).exp() * g;
    }

    let w = x.powi(2).recip();
    ...
    ONE_OVER_SQRT_TWO_PI * (-0.5 * x.powi(2)).exp() * w * (1.0 - g * w)
}
