...
use implied_vol::{black, DefaultSpecialFn, SpecialFn};
...
fn main() {
    ...
    let price = black::price::<MySpecialFn>(
        forward, strike, volatility, maturity, is_call,
    );
    let implied_vol = black::implied_volatility::<MySpecialFn>(
    ...
    let price = black::price::<DefaultSpecialFn>(
        forward, strike, volatility, maturity, is_call,
    );
    let implied_vol = black::implied_volatility::<DefaultSpecialFn>(
        price, forward, strike, maturity, is_call,
    );
    println!("Price: {}", price);
    println!("Implied Volatility: {}", implied_vol);
